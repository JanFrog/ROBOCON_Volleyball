...
class UKF:
    ...
    def __count_transition(self, pos, vel, dt):     #单个维度状态转移的计算函数 (无加速度)
    ...
    def __transition(self, point_previous, dt):     #状态转移函数
        ...
        for pos_x, pos_y, pos_z, vel_x, vel_y, vel_z in point_previous.T:
            ...
            pos_x_new, vel_x_new = self.__count_transition(pos_x, vel_x, dt)
            pos_y_new, vel_y_new = self.__count_transition(pos_y, vel_y, dt)
    ...
    def forward(self, estimated_point, P, observed_point, dt): #滤波器核心
        ...
        try:
            S = np.linalg.cholesky((self.n + self.λ) * P + 1e-6 * np.eye(self.n))   #1e-9 是为了防止除0
